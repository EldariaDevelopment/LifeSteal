...
public class HyperionGive implements CommandExecutor {

    private final LifeSteal plugin;

    public HyperionGive(LifeSteal plugin) {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        player.getInventory().addItem(getHyperion(plugin));
        player.sendMessage("You have been given a Dash Sword!");
        return false;
    }
    public static ItemStack getHyperion(LifeSteal plugin) {
        ItemStack sword = new ItemStack(Material.IRON_SWORD);
        ItemMeta meta = sword.getItemMeta();
        if (meta != null) {
            meta.setDisplayName("§6Hyperion");
            meta.setUnbreakable(true);
            NamespacedKey keys = new NamespacedKey(plugin, "Hyperion");
